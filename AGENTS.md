...
1. [`docs/README.md`](docs/README.md)에서 문서 우선순위와 정본을 확인한다.
2. [`docs/product-scope.md`](docs/product-scope.md)와 [`docs/roadmap.md`](docs/roadmap.md)에서 MVP 포함 여부와 현재 미결정을 확인한다.
3. [`docs/README.md`](docs/README.md#설계-문서화-범위)의 설계 커버리지에서 대상 제품 영역의 현재·목표 정본을 찾는다.
4. [`docs/development-workflow.md`](docs/development-workflow.md)에서 작업을 학습 콘텐츠, 일반 제품 코드, 문서·운영 규칙 중 하나로 분류한다.
5. [`DEC-LESSON-AUTHORSHIP-01`](docs/roadmap.md#2026-09-02-확정-운영-결정)에 따라 학습 교안의 교육 내용은 bam이 작성한다. `learning_document_manager`는 [`docs/lesson-authoring.md`](docs/lesson-authoring.md)와 [`docs/learning-content-design.md`](docs/learning-content-design.md)를 기준으로 전달받은 교안의 접수·출처·ID·링크·상태와 검증 인계만 관리하고, 교육 내용의 보완 필요 사항은 bam에게 반환한다.
6. UI 색상·theme·focus·상태 표현은 [`docs/designs/visual-design.md`](docs/designs/visual-design.md), Code Quest·코딩테스트·설치형 앱·외부 웹과제는 각각 [`docs/designs/code-quest.md`](docs/designs/code-quest.md), [`docs/designs/coding-test.md`](docs/designs/coding-test.md), [`docs/designs/local-application.md`](docs/designs/local-application.md), [`docs/designs/web-assignments.md`](docs/designs/web-assignments.md)를 먼저 읽는다.
7. 코드·콘텐츠 형식은 [`docs/architecture.md`](docs/architecture.md), [`docs/content-schema.md`](docs/content-schema.md)와 연결된 작성 가이드를 따른다.
...
- 접근성, 키보드 흐름과 모바일 사용성을 완료 조건에 포함한다.
...
- 하나의 통합 작업 묶음을 변경 단위로 본다. 하위 에이전트는 반복 중 자기 영향 범위의 focused 검사만 실행하고, 통합된 최종 상태에서 `npm run check` 전체 gate를 통과시킨다. 실제 실행 명령·결과·미실행 범위를 함께 인계한다.
