...
- [`DEC-LOCAL-EVALUATION-01`·`DEC-JAVA-CODING-TEST-01`·`DEC-JAVA-RUNTIME-01`](docs/roadmap.md#2026-09-02-확정-제품-결정)과 [`DEC-JAVA-VERSION-02`](docs/roadmap.md#2026-09-04-확정-제품-결정)의 공개 로컬 평가·격리·Java 25/preview 금지 경계를 유지한다. JavaScript는 공개 테스트마다 one-shot Worker를 사용한다. Java Quest·CT 실행 PASS는 고정 JDK를 사용하는 검증 커널의 Electron IPC prototype 범위이며 웹/소스 브라우저에서는 작성·저장만 지원한다. 새 소스 전달 방식의 Java 실행 연결은 미결정이고 시스템 JDK·Node HTTP 실행 API를 임의 도입하지 않는다. Quest와 CT의 콘텐츠·UI·진도는 분리하고 Docker·Spring Boot·PostgreSQL·Nginx·계정·원격 채점을 요구하지 않는다. 추가 native 실행은 기존 [재개 조건](docs/work-items/2026-09-15-java-code-quest-runtime.md#격리-실패와-재개-조건)을 따른다.
