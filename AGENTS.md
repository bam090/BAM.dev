...
- bam의 명시적 선호에 따라 프로젝트 변경·수정·개선의 설계 판단은 GPT-6 Astra를 명시 지정하고 추론 강도는 기본 `medium`, 필요시 상향한다. 구현·검증은 난이도에 맞는 별도 역할에 배정하며 역할 독립성·승인 경계를 유지한다.
...
- 작업 전에 하위 에이전트별 목표·소유 파일/경로·제약·완료 기준을 간결하게 지정한다. 겹치는 경로는 병렬로 수정하지 않고 선행 인계의 변경 범위와 남은 문제를 확인한 뒤 순차 인계한다.
- 총괄은 위임한 상세 조사를 되풀이하지 않고 근거가 포함된 인계로 진행·반환을 판단한다. 부족하거나 충돌하는 근거만 추가 확인하며, 독립 검증과 중요한 통합 판단은 유지한다. 위임·검토·증거 재사용의 상세 경계는 [`개발 워크플로`](docs/development-workflow.md#읽기-전용-총괄과-임시-역할-운영)를 따른다.
...
- 실행하지 않은 결과를 지어내지 않는다. [`DEC-PUBLIC-EVALUATION-01`](docs/roadmap.md#2026-08-29-확정-제품-결정)에 따라 BAM.dev 앱 결과에는 해당 실행 묶음의 공개 평가만, 외부 Git 웹과제에는 고정된 실습 묶음의 공개 로컬 검증만 사용한다. 비공개·숨김 테스트와 원격 추가 채점은 사용하지 않는다.
- [`DEC-LOCAL-EVALUATION-01`·`DEC-JAVA-CODING-TEST-01`·`DEC-JAVA-RUNTIME-01`](docs/roadmap.md#2026-09-02-확정-제품-결정)과 [`DEC-JAVA-VERSION-02`](docs/roadmap.md#2026-09-04-확정-제품-결정)의 공개 로컬 평가·격리·Java 25/preview 금지 경계를 유지한다. JavaScript는 공개 테스트마다 one-shot Worker를 사용한다. Java Quest·CT는 고정 JDK를 사용하는 Electron IPC prototype에 더해, 명시적 portable 준비와 `dev:java` opt-in 연결을 사용한 검증 커널의 Chrome에서 공개 평가·취소를 확인했다. 기본 `npm run dev`와 GitHub Pages에는 Java 실행 API가 없고 다른 OS·브라우저의 지원 완료도 아니다. 실제 범위와 증거는 [소스 Java 구현 카드](docs/roadmap.md#2026-09-24-소스-java-브라우저-구현-승인과-착수)를 따른다. 시스템 JDK fallback이나 임의 Node 실행 API로 확대하지 않는다. Quest와 CT의 콘텐츠·UI·진도는 분리하고 Docker·Spring Boot·PostgreSQL·Nginx·계정·원격 채점을 요구하지 않는다. 추가 native 실행은 기존 [재개 조건](docs/work-items/2026-09-15-java-code-quest-runtime.md#격리-실패와-재개-조건)을 따른다.
...
- [`DEC-JAVA-IMPLEMENTATION-01`](docs/roadmap.md#2026-09-04-확정-제품-결정)에 따라 Java는 BAM.dev 제품 구현 언어이며 최소 Java 제품 구성요소인 코딩테스트 로컬 runner는 Java 25로 작성한다. 이는 학습자 소스의 Java 25 기준과 별도 계약이고 Spring Boot 사용 승인이 아니다. 별도 로컬 prototype에 Java 제품 runner·Electron 후보가 있으며 검증 커널에 한정한 Java Quest prototype 실행을 독립 PASS했다. 후보 작성과 Java 실행 지원·정식 설치 완료를 구분하며 `DEC-JAVA-RUNNER-01`의 남은 계약을 임의로 확정하지 않는다.
- 접근성과 키보드 흐름은 변경 영향에 맞게 확인한다. 플랫폼별 구현·검사와 완료 판정은 승인된 대상에 한정하며, 제품 전체 모바일 지원 목표를 모든 개별 작업의 완료 조건으로 확대하지 않는다.
- [`DEC-FRONTEND-01`](docs/roadmap.md#2026-09-04-확정-제품-결정)에 따른 목표 UI는 HTML·CSS 기반의 React·TypeScript이며 JavaScript도 유지한다. 현재 HTML·CSS·Vanilla JavaScript와 검증된 Worker·도메인 로직을 전면 재작성하지 않고, `DEC-FRONTEND-MIGRATION-01`에서 버전·빌드·정적 번들·CSP·Worker·첫 이관 경계를 확정한 뒤 작은 화면부터 단계적으로 옮긴다. [`DEC-SOURCE-DISTRIBUTION-01`](docs/roadmap.md#2026-09-22-소스-전달-방식-정정)에 따라 주 전달 방식은 GitHub 소스 clone/다운로드 후 기존 소스 실행이다. 설치형·설치 후 오프라인·DMG·공증·OS 선택을 필수 release gate로 두지 않는다. 로컬 실행·서버/계정 없는 사용·개인 자료 보호는 유지하며 오프라인 품질 범위는 소스 전달 방식에 맞춰 재정의한다.
- `[대체됨]` 설치형을 기본 목표로 둔 DEC-DESKTOP-PROTOTYPE-01의 후속 gate는 DEC-SOURCE-DISTRIBUTION-01로 대체됐다. 기존 Electron·DMG 코드와 검증은 보존하되 새 설치 요청 없이 설치 지원을 기본 계획으로 재개하지 않는다. prototype 환경을 소스 브라우저의 Java 실행·다중 탭 저장 보장으로 확대하지 않는다.
...
- [`DEC-WEB-ASSIGN-01`](docs/roadmap.md#2026-08-29-확정-제품-결정)에 따른 웹과제는 별도 Git 실습 폴더에서 수행한다. [밤위키 원본 활용 결정](docs/roadmap.md#2026-09-15-웹과제-원본-활용-결정)에 따라 밤위키에서 만든 과제를 원본으로 사용한다. 원본·사용자 풀이를 보존하고, 과제별 고정 시작 버전·BAM 연결·공개 검증이 확인되기 전에는 반입·지원 완료를 주장하지 않는다.
