...
`[현재 사실]` 대표 세 문제를 등록하고 배열 데이터/비활성 실행 후보·Java 작성/저장 UI를 반영했다. 독립 콘텐츠 검토와 등록 데이터·schema 검사는 PASS이며 실제 Java 실행은 계속 BLOCKED다. 현재 진행·자체/독립 검증·미실행·최종 인수 경계는 [편입 카드](../work-items/2026-09-15-algorithm-bridge-quests.md#검사와-인계-기록)를 따른다. 이번 편입은 로컬 미게시 상태다.
...
`[확정 결정]` 2026-09-15 bam의 전체 적용 요청은 앞선 대표 세 문제만 편입하던 범위를 대체한다. 원본 72개 중 기존 ARR-01·ARR-02·QUE-01을 그대로 보존하고 나머지 69개를 같은 Java 컬렉션 뒤에 추가한다. 원본의 13개 주제·L1~L4 지원 수준과 공개 판정 근거를 보존하며 문제별 학습 경험·출처·검증은 [전체 편입 카드](../work-items/2026-09-15-algorithm-bridge-all-quests.md)에 기록한다. 이 절은 편입 목표 계약이며 구현·내용 승인·Java 실행 완료를 뜻하지 않는다.
...
`[현재 사실]` 전체 신규 콘텐츠를 등록하고 위 draft 계약·읽기/작성 UI를 반영했다. 독립 콘텐츠 검토·등록 검사·선정 정적 테스트는 PASS이며 데스크톱·문서·통합 판정은 [전체 편입 카드](../work-items/2026-09-15-algorithm-bridge-all-quests.md#검사와-인계-기록)를 따른다. Java 실행 BLOCKED와 로컬 미게시 상태를 유지한다.
...
콘텐츠가 없는 과정은 완료 가능한 항목처럼 표시하지 않는다. Java Code Quest의 첫 편입은 `DEC-JAVA-QUEST-RUNTIME-01`로 승인됐으며 등록·작성은 위 [후속 편입 계약](#algorithm-bridge-대표-세-문제-편입)으로 제공하고, 실행은 capability와 독립 실행 검증이 갖춰질 때 활성화한다. Java 코딩테스트가 MVP 목표라는 `DEC-JAVA-CODING-TEST-01`만으로 Java Code Quest나 정식 Java 과정을 자동 포함한 것은 아니다.
