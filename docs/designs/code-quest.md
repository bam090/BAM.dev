...
`[현재 사실]` 힌트 스크롤 보존을 구현했고 독립 focused 26/26과 대표 데스크톱 검증을 통과했다. Chrome 1440×1000 어두운 theme의 HTML `document-structure`에서 힌트 1·2·3 공개마다 위치와 source를 보존했으며 새 힌트 초점·마지막 버튼 비활성도 확인했다. stale 비동기 처리의 무영향은 자동 회귀 검사로 확인했다. 후속 게시 이력은 [PR #20 기록](../work-items/2026-09-13-github-publication.md#후속-게시-기록-2026-09-15)으로 연결하며 상세 조건·hash·미실행 범위는 작업 카드를 따른다.

### 개념별 직접 작성 경험 확장

`[확정 결정]` 2026-09-14 bam의 요청에 따라 기존 JavaScript·HTML·CSS 개념에서 직접 구현할 기회가 부족한 부분을 Code Quest로 확장한다. 문제 수를 먼저 정하지 않고, [작성 계약](../code-quest-authoring.md)과 [경험 카드](../work-items/2026-09-14-concept-code-quests.md)에서 기존 문제와 다른 A/E/C/T 및 지원 수준을 먼저 검토한다. 교안 본문·객관식·코딩테스트를 다시 쓰거나 Java·SQL 실행기를 추가하는 범위가 아니다.

문제 첫 설명은 만들 결과와 그 쓰임을 말하고, 입력의 타입·허용 범위·빈 값·중복·원본 보존 등 실제 평가 전제를 명시한다. 학습자는 설명과 예시만으로 무엇을 만들지, 어디서 시작할지, 어떤 공개 결과면 완료인지 자신의 말로 설명할 수 있어야 한다. starter는 작성 위치와 함수·마크업·스타일 계약을 제공하고 핵심 판단을 남긴다. 힌트는 개념 회상→관찰할 값·관계→구현 순서로 지원하며 앞 단계에서 완성 답안을 노출하지 않는다. 출력과 관계없는 특정 문법 사용 요구는 평가가 관찰할 수 있는 범위와 구분한다.

새 문제는 기존 언어별 마지막 order 뒤에 추가하고 기존 Quest의 ID·slug·order·revision·source와 사용자 초안·완료 이력을 보존한다. 실제 교안·concept 연결을 통해 현재 탐색기와 지도에 나타나며 스키마나 표시 projection을 새로 만들지 않는다. 추가에 따라 전체 진도의 분모가 늘어날 수 있지만 기존 완료 기록을 취소하거나 이동하지 않는다. 마이그레이션은 없으며 되돌릴 때도 이번 추가 레코드·fixture·영향 테스트만 제거하고 사용자 상태를 초기화하지 않는다.

평가는 기존 JavaScript one-shot Worker, inert HTML DOM, CSS sandbox 계약과 공개 로컬 사례를 재사용한다. HTML 파서 복구나 CSS 계산값의 통과를 포괄적인 문법·접근성 준수로 표현하지 않는다. 외부 요청·의존성·서버를 추가하지 않으며 기존 정적 제공 경계를 유지한다. 새 문제의 대표 데스크톱 흐름에서 설명→편집→힌트→공개 결과→교안/다음 문제 연결과 키보드 접근을 확인한다. 모바일·설치 지원 검증은 이번 범위가 아니다.

`[현재 사실]` 개념별 신규 콘텐츠는 독립 콘텐츠·실행·문서 최종 재검·로컬 통합 PASS를 인수했다. 대상 ID·공개/독립 사례·대표오답·반환 수정·hash와 범위는 [확장 작업 카드](../work-items/2026-09-14-concept-code-quests.md), PR #21 병합 이력은 [게시 기록](../work-items/2026-09-13-github-publication.md#후속-게시-기록-2026-09-15)을 따른다. 다른 주제의 전체 검사나 빌드를 자동 반복하지 않는다.

### Java 직접 작성과 로컬 실행 pilot

`[확정 결정]` 2026-09-15 bam은 문제 설계만 준비하는 대신 **Java 실행 지원**을 선택했다. [DEC-JAVA-QUEST-RUNTIME-01](../roadmap.md#2026-09-15-java-실행-지원-결정)에 따라 첫 Java Quest 한 개를 기존 탐색·편집·힌트·공개 결과·진도에 연결하고 필요한 로컬 prototype을 진행한다. 앞선 데스크톱 탐색/JS·HTML·CSS 확장의 Java 실행 제외는 해당 과거 작업의 경계이며 이번 pilot에 다시 적용하지 않는다.

첫 문제 `quest-java-total-price`는 `java-concept-numeric-operations`의 int 곱셈 범위 판단을 재사용해 상품 소계와 수량에 독립적인 배송비를 조합한 long 값을 직접 반환한다. 메서드 본문을 첫 작성 위치로 보여 주며 공개 사례와 단계별 힌트로 스스로 수정하도록 돕는다. 학습 사다리·새 E·기존 객관식과의 차이·대표오답은 [작업 카드](../work-items/2026-09-15-java-code-quest-runtime.md)에 기록하고 독립 콘텐츠 검토를 받는다. 단순히 Java 언어가 늘었다는 이유로 새 A/C나 전이 성과를 주장하지 않는다.

실행 후보는 [ADR 0005](../decisions/0005-java-quest-local-runtime.md)의 번들 JDK·공개 테스트마다 새 JVM·좁은 IPC를 사용한다. JavaScript Worker나 Java 코드의 JS 번역으로 채점하지 않는다. 앱 shell에서 `java-static-method-v1` capability가 준비된 경우에만 Java 실행을 제공한다. 일반 웹에서는 기존 세 언어 실행을 유지하고 Java 실행 버튼/완료 가능한 활성 과정으로 오인하게 하지 않는다. Java deep link는 아래 편입 계약에 따라 문제·편집·저장을 제공하되 실행 준비 중임을 안내하고 없는 runner로 넘기지 않는다.
...
`[확정 결정]` bam의 2026-09-15 대표 ARR-01·ARR-02·QUE-01 편입 승인에 따라 원본 문제의 직접 작성 경험을 Java Code Quest로 옮긴다. 기존 Java pilot과 기존 세 언어 Quest를 보존하고 신규 3개를 뒤에 추가한다. 별도 Algorithm Bridge 제품·과정·코딩테스트로 합치지 않고 실제 Java 배열·조건/반복·Deque 교안에 연결한다. 문제별 경험 카드·출처·원본 보존과 검증 상태는 [작업 카드](../work-items/2026-09-15-algorithm-bridge-quests.md), 타입·공개 관찰은 [콘텐츠 계약](../content-schema.md#algorithm-bridge-java-배열-quest-편입)이 정본이다.

`[대체됨]` 앞선 pilot의 capability=false일 때 Java 컬렉션과 상세를 모두 숨기던 중단 방식은 이번 편입의 읽기·작성 흐름에 한해 대체한다. Java 실행 지원 차단과 [격리 재개 조건](../decisions/0005-java-quest-local-runtime.md#현재-실행-판정과-재개-조건)은 그대로 유지한다. 원본 문제를 읽고 개념·관찰 지점을 생각한 뒤 코드를 작성·저장하는 경험은 실제 JVM 실행 없이 제공할 수 있기 때문이다.

목록·관련 교안·검색·지도에서 Java 등록 문제를 찾고 상세의 지문→예시→편집→순차 힌트→초안 저장을 이용한다. capability가 false이거나 bridge가 없는 일반 웹에서는 목록과 상세에 **Java 실행 준비 중 · 코드 작성·저장 가능**을 명시하고 실행 버튼은 비활성화한다. 실행 가능한 문제 수·완료 가능한 활성 과정과 혼동되는 표시를 피한다. 긴 배열은 앞 20개·전체 개수로 표시하고 사용자 클릭으로 전체 공개 원본 JSON을 다운로드하게 한다. 큰 원문을 textarea 등 DOM에 주입하지 않고 Blob/objectURL을 정리한다. 빈 배열·최대 길이·원본 보존·새 참조 등 공개 판정 조건은 숨기지 않는다.

UI 비활성화와 별개로 submit/실행 진입점도 capability를 확인해 runner 호출과 완료 쓰기를 차단한다. 없는 실행기를 JS 번역·문자열 검사·예시 출력으로 대신하지 않는다. 차단된 실행은 PASS·실패한 답안·완료로 만들지 않으며 미실행 상태를 유지한다. capability 지원을 회복했을 때만 설치본의 모든 공개 사례와 관찰 통과를 완료로 기록한다.

신규 문제는 안정 ID별 기존 Quest 저장 구조에 새 초안을 쓰며 pilot과 기존 28개 완료/초안·revision·키를 덮어쓰지 않는다. 별도 저장소나 사용자 상태 마이그레이션을 만들지 않는다. 읽기·탐색·힌트만으로 완료를 쓰지 않으며 저장된 Java 초안은 같은 origin의 재진입·새로고침에서 복원한다. 다른 origin의 브라우저/설치앱 진도 자동 이전은 하지 않는다.
...
`[현재 사실]` 후속 전환에서 신규69를 Java 코딩테스트로 옮겼고 Java pilot·준비3을 포함한 기존32 Quest는 보존했다. 신규CT의 독립 콘텐츠 검토는 PASS이며 실행/화면·통합·게시 상태는 [전환 카드](../work-items/2026-09-15-algorithm-bridge-coding-tests.md)를 따른다.

`[대체됨]` 아래는 전체69를 Quest로 등록한 당시 계약·검사 이력이다. 최신 [코딩테스트 전환](coding-test.md#algorithm-bridge-코딩테스트-전환)은 원본72를 CT로 제공하고 기존32 Quest만 보존한다. 따라서 전체69의 Quest 상시 등록·기존 카드의 Git 제외는 현재 전환 gate가 아니다. 옛 URL·초안의 보존과 명시적 가져오기는 전환 계약을 따른다.

`[대체됨]` 다음은 당시 전체 Quest 편입 계약이다. 2026-09-15 bam의 전체 적용 요청은 앞선 대표 세 문제만 편입하던 범위를 대체한다. 원본 72개 중 기존 ARR-01·ARR-02·QUE-01을 그대로 보존하고 나머지 69개를 같은 Java 컬렉션 뒤에 추가한다. 원본의 13개 주제·L1~L4 지원 수준과 공개 판정 근거를 보존하며 문제별 학습 경험·출처·검증은 [전체 편입 카드](../work-items/2026-09-15-algorithm-bridge-all-quests.md)에 기록한다. 이 절은 편입 목표 계약이며 구현·내용 승인·Java 실행 완료를 뜻하지 않는다.

학습자는 연결 교안에서 알고리즘 과정의 문제를 찾고 지문·생각 질문·typed 예시·원본 공개 테스트를 읽은 뒤 코드를 작성하고 초안을 저장한다. 과정은 `lessonId → courseId` 관계로 정하며 기존 세 문제의 Java 교안 연결은 바꾸지 않는다. 신규 문제는 검증된 알고리즘 교안에 연결하고 직접 대응 교안이 없으면 실제 선수 교안이라는 한계를 카드에 밝힌다. 언어는 Java이며 별도 Algorithm Bridge 제품이나 통합 필터를 만들지 않는다. 원본 slot은 출처 대응표에서 식별하고 원본의 순서와 기존 Quest의 order/화면 번호를 같다고 설명하지 않는다.

신규 69개는 `executionMode: "draft-only"`이다. **코드 작성·저장 가능 / 이 문제의 Java 실행은 지원하지 않음**을 구분해 표시한다. 기존 네 문제는 런타임 실패로 실행 준비 중인 상태를 유지한다. 공통 capability가 나중에 true가 되어도 draft 문제는 실행하지 않는다. UI·submit·request builder·adapter는 capability와 기존 네 문제 allowlist, draft 제외를 모두 확인하며 runner 호출·시도/완료 생성으로 진행하지 않는다. source 문자열 검사·예시 출력·JS 변환으로 실제 Java 평가를 흉내 내지 않는다. 신규 타입의 향후 실행 프로토콜은 이번 승인에 포함하지 않는다.

공개 테스트는 [draft 콘텐츠 계약](../content-schema.md#algorithm-bridge-전체-편입의-draft-계약)에 따라 원본 테스트 전체와 사례별 소스를 우선 열람한다. 반복 생성된 최대 입력·helper·허용 오차·입력 불변·참조 동일성 조건을 typed 예시로 축소하지 않는다. 코드는 escape된 텍스트로 제공하고 큰 공개 데이터는 기존 전체 다운로드 흐름을 사용한다. 해결 코드 fixture는 개발 경로에만 두며 앱에 포함하지 않는다. 원본의 지원이 없는 L3/L4에 정답 유도 힌트를 추가하지 않는다. 힌트가 0개이면 빈 단계/활성 힌트 버튼을 만들지 않으며 원본 지원 수 0~5개만 순서대로 제공한다.

기존 네 Java 객체·28개 다른 언어 Quest·안정 ID·revision·초안·완료와 저장 키를 보존한다. 신규 order는 5~73이며 기존 ID로 병합하거나 저장 마이그레이션을 하지 않는다. 읽기·힌트·저장은 완료 근거가 아니다. curriculum·런타임 supervisor·Java 프로세스를 바꾸지 않고 새 의존성·원격 요청을 추가하지 않는다. 번들된 문제/공개 소스와 같은 origin의 저장을 사용하며 설치/오프라인 지원 완료를 새로 주장하지 않는다.

검증은 69개 스키마·타입/예시·ID/교안 연결·원본 공개 소스와 경험 카드의 독립 검토, 기존 32개 보존과 원본 229개 파일 hash, 대표 데스크톱의 탐색/원본 열람·escape·0힌트·편집/복원·키보드·capability=true 강제 조건에서도 draft 실행 차단·완료 불변을 포함한다. Java/JDK/javac·Electron·빌드·프로세스 종료 실험·Git은 수행하지 않는다. 실제 Java 평가·Java 25 호환은 `BLOCKED/미검증`이며 모바일·다른 OS·패키징은 범위 밖이다. 실제 PASS와 미실행·후속 통합은 전체 편입 카드의 receipt로만 판정한다.

`[대체됨]` 당시 전체 신규 콘텐츠를 등록하고 위 draft 계약·읽기/작성 UI를 반영했다. 독립 콘텐츠 검토·등록 검사·선정 정적 테스트는 PASS이며 데스크톱·문서·통합 판정은 [전체 편입 카드](../work-items/2026-09-15-algorithm-bridge-all-quests.md#검사와-인계-기록)를 따른다. Java 실행 BLOCKED와 로컬 미게시 상태를 유지한다.
...
예시는 `CSS / 선택자 / 3. 클래스 선택자`다. `languageId`는 작성·평가 계약을 정하고 `courseId`는 학습 과정을 구분한다. 학습 과정과 작성 언어가 같거나 실행기를 공유하더라도 과정은 별도로 식별한다. 현재 알고리즘 교안의 Java 기준과 JavaScript 실행 지원을 혼동하지 않는다.
...
`[확정 결정]` 2026-09-15 사용자의 예제 읽기 불편 개선 승인에 따라, 함수형 JavaScript·Java Quest의 기존 예제와 공개 사례를 읽기 쉬운 표시로 재배치한다. 학습 내용·사례·기대값을 다시 쓰는 작업이 아니다. 구현·검증 상태와 참고 출처는 [작업 카드](../work-items/2026-09-15-code-quest-example-readability.md)에 기록한다.

- 기본 예제는 `예제 번호 → contract.parameters 순서의 실제 매개변수명 → 반환값` 열을 갖는 의미 있는 표로 제공한다. 입력과 기대 반환값을 같은 행에서 비교하며, 원래 설명은 표 바로 아래 `예제 1 설명`처럼 번호로 대응한다. 설명이 없는 예제에 해설을 생성하지 않는다.
- JSON 값의 타입을 보존한다. 배열의 대괄호, 문자열의 따옴표, 빈 배열·객체, `null`, boolean을 유지한다. Java의 decimal string은 반환 선언 타입이 `long`인 기대값에 한해서만 숫자처럼 표시하고 배열을 `String`으로 강제 변환하지 않는다. 현재 Java 매개변수 계약은 `int`·`int[]`이며 이번 작업에서 지원 서명을 확장하지 않는다. 값은 간결하게 표시하고 쉼표 사이에서 자연스럽게 줄바꿈한다.
- 큰 배열은 앞 20개 원소와 정확한 전체 개수를 구분해 안내할 수 있다. 생략 안내를 가짜 문자열 원소 `"…"`로 배열에 섞지 않는다. 축약은 표시 전용이며 전체 원본을 확인할 경로를 보존한다.
- Java 공개 테스트는 처음부터 존재와 총개수, 펼치는 방법이 보이는 native `details`로 묶는다. 펼치면 기존 모든 입력·expected·observations와 큰 JSON 다운로드에 접근할 수 있다. 공개 기준의 접힘을 비공개 테스트나 추가 채점으로 표현하지 않는다. 내부 공개 사례도 매개변수명을 사용하고 배열 형태를 보존한다.
- 1024px·1440px 데스크톱에서 반환값 열이 숨거나 페이지 전체가 가로로 넘치지 않도록 한다. 열이 지나치게 많으면 실제 매개변수명을 유지하는 단순 라벨·값 배치를 사용한다. 표에는 열 머리글 의미를 제공하고, 펼치기·다운로드의 키보드 동작과 초점 표시, 밝고 어두운 테마의 대비를 유지한다.

`[확정 결정]` 데스크톱 폭이 1100px 이하이면 읽기 폭을 확보하도록 Quest 문제 다음에 편집기를 한 열로 배치하고, 1101px 이상에서는 두 열을 유지한다. 기존 820px 이하 배치는 유지하며 이번 변경으로 모바일 검증 완료를 주장하지 않는다.

`[확정 결정]` HTML·CSS 작성 예시, 콘텐츠 원문·정답·ID·revision·사례 순서, route·초안·힌트·저장·평가기와 Java 실행 BLOCKED 상태는 보존한다. 스키마·데이터 마이그레이션과 외부 의존성은 필요 없으며 정적 번들 안에서 오프라인으로 읽는다. Java/JDK/Electron 실행·빌드와 Git 게시, 편집기 전체 재설계, 모바일 지원 판정은 이번 범위가 아니다. 결함은 표시 변경만 반환·수정하며 원본 콘텐츠로 우회하지 않는다.

완료는 대표 JS·Java 예제의 매개변수명·순서·타입·설명 대응, 전체 공개 데이터 접근, 최초 화면의 스크롤 감소와 데스크톱 반환값 가시성, 키보드·테마, 기존 초안·힌트 및 Java 차단 보존을 실제 focused 검사로 확인한 뒤 판정한다. 새 A/E/C/T 학습 기회를 추가하지 않는 일반 제품 UI 변경이며, 학습 내용을 바꿔야 할 필요가 발견되면 해당 변경을 별도 작업으로 반환한다.
