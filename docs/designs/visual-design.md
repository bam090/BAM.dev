...
이 절의 최초 적용·검사·게시 범위는 2026-09-13 작업 이력이다. 후속 전체 화면 적용은 아래 [전체 화면의 서비스 사이드바 통일](#전체-화면의-서비스-사이드바-통일)의 범위와 변경 영향 검사 계약을 따른다.
...
`[확정 결정]` 2026-09-14 bam의 “사이드바 밤데브 학습문서랑 객관식에 적용된 사이드바로 전체 적용해줘”에 따라 기존 236px 서비스 사이드바를 모든 앱 화면에 공통 적용한다. 학습 내용·평가 행동을 바꾸지 않는 일반 제품 UI 변경이다. 현재와 검증 상태는 [전체 적용 작업 카드](../work-items/2026-09-14-global-service-sidebar.md)에 기록한다.

`[현재 사실]` 제품 구현과 독립 focused·대표 데스크톱 검증을 통과했다. Chrome 1440×1000의 12개 route에서 236px 공통 shell·서비스 메뉴 6개·정확한 현재 표시·가로 넘침 없음을 확인했다. 비동기 목록 준비 후 현재 문맥이 잘못되는 최초 실패를 수정하고 재검증했다. 최종 독립 문서 검토·통합도 PASS이며 모바일·전체 검사·빌드는 이번에 실행하지 않았다. 상태 보존·재사용 검사와 상세 증거는 작업 카드에 둔다.

- 브랜드·문서/문제 검색·서비스 이동·현재 위치·최근 본 문서 최대 2개·하단 theme을 같은 shell로 제공한다. 기존 문서/객관식의 주제·읽기 복귀·문제 토큰 계약과 Quest 탐색을 보존하고 코딩테스트·기존 Web Project·내 학습 기록 등 기존 목적 화면도 같은 서비스 탐색에서 접근한다. 검색의 기존 문서/문제 범위는 이번에 확장하지 않는다.
- 현재 서비스 표시는 한 항목에만 부여한다. 문서/주제의 하위 현재 표시는 서비스 상태와 구분한다. 본문·편집기·결과와 겹치거나 가로 넘침을 만들지 않으며 긴 메뉴는 하단 theme 접근을 유지한다.
- 서비스 메뉴의 기존 홈·학습문서·객관식·Quest에 코딩테스트·웹과제의 실제 진입 링크를 추가하고 내 학습 기록은 하단에 유지한다. 학습문서/객관식 밖의 문맥을 임의로 학습문서로 표시하지 않는다. 로딩·빈 결과·오류·fallback에서도 같은 shell을 사용하되 콘텐츠 목록이 아직 없어도 안전하게 표시한다. 목록의 비동기 준비가 끝나도 현재 서비스를 바꾸거나 본문을 다시 렌더링하지 않는다.
- 기존 과정·교안·복습·실습의 유효 링크는 공통 사이드바 또는 해당 본문의 문맥 탐색에서 계속 접근 가능해야 한다. 준비 중인 항목을 새 활성 서비스로 만들지 않는다. 제품별 콘텐츠·route·ID·진도·초안·실행/제출 내역·저장 키와 공개 평가기를 변경하지 않으며 마이그레이션은 없다.
- 검색 입력·검색 열기/닫기·theme 조작은 본문을 다시 렌더링하거나 진행 중 풀이·편집·실행을 초기화하지 않는다. 실제 서비스 이동은 각 화면의 기존 저장·복귀·실행 정리 계약을 사용한다. 이름 있는 탐색·검색·theme과 보이는 키보드 focus, 비색상 현재 표시를 유지한다.
- 데스크톱만 새 적용·대표 검증 대상으로 삼는다. 기존 모바일 fallback을 보존하며 모바일 새 UI·검사, 콘텐츠·SQL·Java runner·설치 앱·React 이관·서버/DB/계정·새 의존성·외부 요청·Git 게시를 포함하지 않는다.

완료는 모든 route의 공통 shell·서비스 현재 표시와 기존 링크를 focused 검사로 확인하고, 대표 데스크톱에서 밝게/어둡게·키보드·넘침·검색/theme 전후 본문과 상태 보존·실습 초안/실행/내역을 확인하는 것이다. [공통 검증과 재검증](../development-workflow.md#공통-검증과-재검증)에 따라 직접 변경 영향만 선정한다. 앞선 시안의 전체 검사·Git 게시 문구는 이번 작업 gate가 아니며, 실패는 코드·스타일 또는 해당 증거 담당에게 반환한다.
