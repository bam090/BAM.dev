...
`[현재 사실]` 이 문서는 기존 인앱 HTML·CSS 과제의 유효한 평가·저장 계약이다. 앞으로 사용할 밤위키 원본 외부 과제의 연결은 [별도 설계](../designs/web-assignments.md)를 따른다. 외부 연결·검증 완료 전에는 기존 콘텐츠·초안·진도를 삭제하거나 이 계약이 자동 폐기됐다고 해석하지 않는다.

- 상태: 승인
- 결정일: 2026-08-18
- 적용 범위: 6차 Web Project v1

> `[현재 사실]` 이 ADR은 현재 인앱 HTML·CSS Web Project에 계속 적용됩니다. 2026-08-29에 확정된 외부 Git 웹과제 방향은 [`../designs/web-assignments.md`](../designs/web-assignments.md)에서 별도로 설계하며, 기존 기능의 병행·이관·종료 결정 전까지 이 ADR을 대체 완료로 표시하지 않습니다.
...
- 브라우저에 전달된 모든 assertion은 공개 평가이며, 학습자 결과에는 이 공개 기준만 사용하고 비공개·숨김 테스트나 원격 추가 채점을 사용하지 않는다.
...
실제 검토자가 없는데 검증된 결과처럼 보이므로 채택하지 않았다. 원격 검토·계정·서버 운영은 현재 제품 목표가 아니며, 이후 다시 제안하려면 [`DEC-DELIVERY-01`](../roadmap.md#2026-08-29-확정-제품-결정)과의 충돌 및 완전한 로컬 대안을 새 제품 결정으로 먼저 검토한다.
