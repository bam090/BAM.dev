...
## 신뢰 채점 요구가 다시 생길 때의 경계

`DEC-DELIVERY-01`과 `DEC-PUBLIC-EVALUATION-01`에 따라 현재 제품과 로드맵은 서버 채점·비공개 테스트를 사용하지 않는다. `run(request, { signal })`과 `GradeReport` 포트는 로컬 평가를 교체하기 위한 미래 계획이 아니라 UI와 실행 세부를 분리하는 현재 내부 경계다.

공인 인증·보상·대회처럼 변조 방지와 감사 가능한 결과가 새로 필요해지면 이는 현재 제품 결정과 충돌한다. 별도 제품 결정에서 운영 부담과 로컬 대안을 먼저 검토하고, 서버가 필요하다고 확정된 뒤에만 승인된 테스트 조회, 프로세스·컨테이너 격리, CPU·메모리·네트워크·파일시스템 제한과 실행 기록을 새 ADR로 설계한다.
