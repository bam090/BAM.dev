...
1~5차는 외부 채점 서버 없이 로컬에서 동작해야 합니다. 동시에 학습자 HTML·CSS를 BAM.dev 주 문서에 그대로 삽입하거나 실행해서는 안 되며, 학습자 결과에 비공개·숨김 테스트나 원격 추가 채점을 사용하지 않습니다.
...
현재 결과는 로컬 학습 피드백에만 사용하고 공인 인증·보상·정식 채점을 제공한다고 표현하지 않습니다. 그런 신뢰 보장이 나중에 요구되면 현재 제품 결정과 충돌하므로 별도 제품 결정이 먼저 필요하며, 서버 격리 실행기는 현재 MVP의 계획이나 요구사항이 아닙니다.
