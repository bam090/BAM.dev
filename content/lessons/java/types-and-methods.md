...
> 이 교안은 Java 콘텐츠 구조를 확인하기 위한 읽기·추론 샘플입니다.
> 현재 페이지에서 Java 코드를 실행하거나 채점하지 않습니다.
> 실행하지 않은 결과를 맞다고 가정하지 말고, 먼저 타입 관계를 설명해 보세요.
...
`level`은 `int` 값, `completed`는 `boolean` 값을 담습니다.
타입은 변수가 가질 수 있는 값과 그 값에 허용되는 연산을 제한하여, 맞지 않는 사용을 컴파일 단계에서 찾는 데 도움을 줍니다.

### 참조 타입: 객체를 가리키는 참조 값을 다루는 종류

클래스 타입, 인터페이스 타입, 배열 타입은 참조 타입입니다.
`String`은 클래스 타입이고 `int[]`는 배열 타입이므로 둘 다 참조 타입입니다.
...
`courseName`과 `scores`에는 각각 객체를 가리키는 참조 값이 들어갑니다.
참조 타입 변수에는 `null`이 들어갈 수도 있으므로, 그 참조로 객체의 기능을 사용하기 전에 실제 객체를 가리키는지 확인해야 하는 상황이 있습니다.

### 배열을 나중에 만들 때

`int[] nums;`는 배열을 만드는 코드가 아닙니다.
`int` 배열을 가리킬 변수 `nums`만 선언합니다.

`{1, 2, 3}`처럼 배열의 처음 값을 적는 문법은 변수 선언과 한 문장일 때 바로 쓸 수 있습니다.

```java
int[] nums = {1, 2, 3}; // 가능
```

선언이 이미 끝난 뒤에는 중괄호만 대입할 수 없습니다.
...
이때는 `new int[]`로 새 배열을 만든 뒤 대입합니다.

```java
int[] nums;
nums = new int[] {1, 2, 3}; // 가능
```

한 줄로 기억하면 됩니다.
선언과 동시에 값을 넣을 때는 `{...}`, 나중에 배열을 만들어 넣을 때는 `new int[] {...}`를 사용합니다.

기본 타입과 참조 타입의 차이를 단순히 “작은 값과 큰 값”으로 나누면 안 됩니다.
핵심은 변수가 **기본 값**을 담는지, 객체를 가리키는 **참조 값**을 담는지입니다.

## 메서드 선언을 입력·출력 계약으로 읽기

다음 메서드는 점수와 통과 여부를 받아 안내 문장을 만듭니다.
아직 실행하지 말고 선언부부터 읽어 보세요.
...
여기서 `80`과 `true`는 인수이고, 각각 선언된 `int`, `boolean` 매개변수에 대응합니다.
호출 결과를 받는 `message`의 타입도 메서드의 반환 타입인 `String`과 맞습니다.
...
반환 타입이 `void`라면 결과 값을 돌려주지 않는 메서드입니다.
반환 타입이 `void`가 아니라면 메서드가 정상적으로 끝나는 경로에서는 선언한 타입과 맞는 값을 `return`해야 합니다.
...
컴파일 오류를 만났을 때 강제로 형 변환부터 시도하지 마세요.
먼저 매개변수 타입, 반환 타입, 실제 인수와 `return` 표현식의 타입을 차례로 비교하면 계약이 어긋난 지점을 좁힐 수 있습니다.
...
- [Java Language Specification SE 26 §10.2: 배열 변수](https://docs.oracle.com/javase/specs/jls/se26/html/jls-10.html#jls-10.2)
- [Java Language Specification SE 26 §10.6: 배열 초기화식](https://docs.oracle.com/javase/specs/jls/se26/html/jls-10.html#jls-10.6)
...
`int`와 `boolean`은 Java가 미리 정의한 기본 타입입니다.
`String`은 클래스 타입이고 `int[]`는 배열 타입이므로 둘 다 참조 타입입니다.

### 답변 2

참조 타입 변수는 객체 자체를 담는 것이 아니라 그 객체를 가리키는 참조 값을 담습니다.
참조 타입 변수에는 아무 객체도 가리키지 않는 `null`이 들어갈 수도 있습니다.

### 답변 3

`makeResult`의 매개변수 타입은 순서대로 `int`와 `boolean`이고 반환 타입은 `String`입니다.

### 답변 4

`makeResult("80", true)`는 선언된 입력 계약과 맞지 않습니다.
첫 번째 인수 `"80"`은 `String`이지만 첫 번째 매개변수는 `int`를 요구합니다.

### 답변 5

`age >= 19`는 참 또는 거짓을 만드는 비교식이므로 반환 타입은 `boolean`이어야 합니다.
