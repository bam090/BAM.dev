...
개별 요소 이름을 외우는 것만으로는 충분하지 않습니다.
실제 요구사항을 보고 “이 콘텐츠의 역할은 무엇인가?”, “다른 콘텐츠와 어떤 관계인가?”, “화면을 보거나 마우스를 쓰지 않아도 의미와 기능이 남는가?”라고 질문해야 합니다.
...
정답 구조가 하나만 있는 것은 아닙니다.
중요한 것은 선택한 요소가 콘텐츠의 실제 역할과 관계를 설명하며, 그 이유를 자신의 말로 말할 수 있는지입니다.
...
Elements 패널에서 보는 것은 원본 파일 문자열이 아니라 브라우저가 파싱해 만든 DOM입니다.
부모·자식 관계가 예상과 같은지, 브라우저가 잘못된 중첩을 의도와 다르게 복구하지 않았는지 확인합니다.

### 4. Nu HTML Checker

[Nu HTML Checker](https://validator.w3.org/nu/)에 소스나 URL을 입력하여 의도하지 않은 문법·콘텐츠 모델 문제를 찾습니다.
오류를 먼저 고치고 경고는 코드의 문맥과 의도를 확인합니다.

검사 결과가 없다는 사실만으로 정보 구조, 링크 문구, 대체 텍스트, 실제 키보드 흐름이 모두 좋다고 인증되는 것은 아닙니다.
검사기는 실수를 찾는 한 가지 도구입니다.
...
좋아하는 기술과 주간 학습 시간을 3행짜리 표로 만드세요.
`caption`, 열 머리글과 행 머리글을 사용한 뒤 각 데이터 셀의 두 머리글을 말해 보세요.
...
코드를 쓰기 전에 콘텐츠를 영역과 관계로 나누면 요구사항을 빠뜨리거나 보이는 모양만으로 요소를 고르는 실수를 줄일 수 있습니다.
각 요소를 선택한 이유도 더 분명하게 설명할 수 있습니다.

### 답변 2

Elements 패널은 원본 HTML 문자열이 아니라 브라우저가 파싱해 만든 DOM을 보여 줍니다.
브라우저가 잘못된 중첩을 복구하는 과정에서 원본 소스와 DOM 구조가 달라질 수 있습니다.

### 답변 3

Nu HTML Checker는 문법과 콘텐츠 모델 문제를 찾는 도구입니다.
링크 문구가 목적을 잘 설명하는지, 대체 텍스트가 현재 문맥에 맞는지 같은 의미의 품질까지 보장하지는 않습니다.

### 답변 4

`name="question"`을 삭제하면 질문 입력값에 제출 데이터 이름이 없어집니다.
따라서 일반적인 폼 제출 데이터에 그 질문 값이 포함되지 않습니다.

### 답변 5

`action="/questions"`는 폼을 제출할 목적지만 표현합니다.
그 주소에서 요청을 처리하고 질문을 저장할 서버 기능은 별도로 만들어야 합니다.

### 답변 6

자동 검사는 문법처럼 정해진 규칙의 문제를 찾는 데 도움이 되지만 실제 키보드 이동과 조작 경험을 모두 판단하지는 못합니다.
그래서 자동 검사와 함께 키보드로 링크·입력·버튼을 직접 점검해야 합니다.
