...
폼 안에서 `button`의 `type`을 생략하면 제출 버튼으로 동작할 수 있습니다. 제출하지 않는 JavaScript 동작이라면 `type="button"`을 명시합니다. `reset`은 사용자가 작성한 값을 즉시 지우므로 실제 필요성과 복구 가능성을 고려합니다.
