...
앞에서 동작을 선언한 실제 선택자와 같거나 충분한 우선순위로 뒤에서 재정의해야 합니다. 예제에서는 전환 시간뿐 아니라 hover와 focus에 적용된 위치 이동도 제거합니다. 모든 상태 변화를 숨기기보다 배경색처럼 상태를 알리는 변화는 즉시 보여 주고, 장식적인 이동과 반복을 제거하세요.
