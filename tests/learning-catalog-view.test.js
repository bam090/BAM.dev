...
test("문서 목록은 활성 문서만 나열하고 보관 교안의 메타데이터와 깊은 URL은 남긴다", () => {
  ...
  assert.equal(java[0].sample, true);
...
test("문서와 문제 첫 화면은 검색 다음에 주제 선택을 보여 주고 아직 카드를 나열하지 않는다", () => {
  for (const kind of ["learn", "review"]) {
    ...
    for (const id of ["html", "css", "javascript", "java", "spring", "algorithm", "cs", "typescript", "react", "all"]) {
      assert.match(html, new RegExp(`data-catalog-topic="${id}"`));
    }
...
test("JavaScript 주제는 두 언어 과정을 묶고 알고리즘을 별도 주제로 유지한다", () => {
  const javascriptLessons = curriculum.lessons.filter((lesson) => ["javascript", "javascript-notes"].includes(lesson.courseId));
  ...
  assert.ok(javascript.some((item) => item.href.startsWith("#/learn/javascript-notes/")));
...
test("준비 중 주제와 문제 자료 없는 주제는 제공 중인 카드로 오인되지 않는다", () => {
  for (const kind of ["learn", "review"]) {
    const html = renderLearningCatalog({ ...options, kind });
    for (const id of ["spring", "cs", "typescript", "react"]) {
...
test("9개 승인 키워드와 기존 단원 묶음은 문제를 빠뜨리거나 중복 집계하지 않는다", () => {
  const items = getLearningCatalogItems({ ...options, kind: "review" });
  assert.equal(items.reduce((sum, item) => sum + item.count, 0), 52);
  const pilot = items.filter((item) => /\/js-notes-/.test(item.href));
  assert.equal(pilot.length, 9);
  assert.equal(pilot.reduce((sum, item) => sum + item.count, 0), 13);
  assert.equal(pilot.find((item) => item.href.endsWith("concept=js.function-return")).count, 3);
...
test("HTML 목록에서 새 개념 카드와 기존 보완 문항은 12개 문제를 각각 한 번만 가리킨다", () => {
  ...
  for (const concept of concepts.filter((item) => item.documentLessonId)) {
...
test("사용자가 아는 콜백·함수·HTML 검색어로 문서와 문제를 찾는다", () => {
  for (const kind of ["learn", "review"]) {
    ...
    assert.ok(callbacks.some((item) => item.href.includes(kind === "learn" ? "/javascript-notes/functions" : "js-notes-functions")), kind);
...
test("검색어·제목·요약·저장 제목은 HTML로 실행하지 않고 텍스트로 표시한다", () => {
  ...
  unsafe.lessons[0].title = '<script>alert("lesson")</script>';
  unsafe.lessons[0].summary = '<img src=x onerror="alert(1)">';
