...
test("카테고리별 과정과 CS 준비 중 상태를 표시한다", () => {
  ...
  assert.equal((html.match(/class="language-nav-link/g) ?? []).length, 6);
  ...
  const expectedCourses = [
    ...
    ["#/learn/java/types-and-methods", "Java", "샘플"],
  ];
  ...
  assert.deepEqual(
    ...
    "새 원문 과정이 기존 5개 과정의 경로를 대체하거나 중복하지 않습니다.",
  );
